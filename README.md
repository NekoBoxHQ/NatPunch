...
1. 登录 Web 面板，在【客户端】页添加客户端，复制生成的 **VKEY**
2. 在待部署的设备上 SSH 粘贴执行一键安装命令（面板上一键复制）：

```shell
wget -qO- http://你的服务端:28090/install.sh | sh -s -- --openwrt 'VKEY' 你的服务端 8025
```

脚本自动识别架构（amd64/arm64），下载客户端、写入配置、注册系统服务并启动。
